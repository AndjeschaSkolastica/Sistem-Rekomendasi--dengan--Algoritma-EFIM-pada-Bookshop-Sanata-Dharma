...
def proses_upload():
    ...
    try:
        ...
        for file in files:
            ...
            # =====================================
            # TAMPILKAN HASIL PEMBACAAN FILE
            # (20 BARIS PERTAMA)
            # =====================================

            print("\n")
            print("="*80)
            print("DATA HASIL READ EXCEL")
            print("="*80)

            print("Nama File :", file.filename)
            print("Jumlah Baris :", len(df_excel))
            print("Jumlah Kolom :", len(df_excel.columns))
            print()
        ...
        # Simpan Excel juga
        path_dataset_excel = os.path.join(
            DATASET_FOLDER,
            'df_preprocessing.xlsx'
        )
...
def proses_efim():
    ...
    try:
        ...
        # Simpan Excel juga
        path_hasil_excel = path_hasil.replace(
            '.pkl',
            '.xlsx'
        )
        ...
        # Simpan Excel juga
        path_uconf_excel = path_uconf.replace(
            '.pkl',
            '.xlsx'
        )
        ...
        df_model.to_pickle(path_ringkasan)

        # Simpan Excel juga
        path_ringkasan_excel = os.path.join(
            MODEL_FOLDER,
            'ringkasan_model.xlsx'
        )

        df_model.to_excel(
            path_ringkasan_excel,
            index=False
)
